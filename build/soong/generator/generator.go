...
// Copyright (C) 2018 The ZennyOS Project
//
...
func init() {
	android.RegisterModuleType("zenny_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = zennyExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = zennyExpandVariables(ctx, dep_file)
	...
	cmd := zennyExpandVariables(ctx, String(g.properties.Cmd))
